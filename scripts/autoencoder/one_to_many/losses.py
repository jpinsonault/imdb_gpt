...
def _per_sample_field_loss_seq(field: BaseField, pred: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
    ...
    if isinstance(field, TextField):
        ...
        loss_flat = F.cross_entropy(logits.view(B * T * L, V), tgt.view(B * T * L), ignore_index=pad_id, reduction="none").view(B * T, L)
    ...
    if isinstance(field, NumericDigitCategoryField):
        ...
        loss_flat = F.cross_entropy(logits.view(B * T * P, V), tgt.view(B * T * P).long(), reduction="none").view(B * T, P)
...
def many_to_many_loss(
    ...
    n = z_members.size(1)
    with torch.no_grad():
        flat_targets = [y.view(b * n, *y.shape[2:]) for y in targets_seq]
        z_tgt_flat = teacher_encoder(flat_targets)
        z_tgt_seq = z_tgt_flat.view(b, n, -1)
